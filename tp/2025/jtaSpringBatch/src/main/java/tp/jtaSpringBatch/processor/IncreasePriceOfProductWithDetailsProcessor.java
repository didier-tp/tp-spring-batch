...
public class IncreasePriceOfProductWithDetailsProcessor implements ItemProcessor<ProductWithDetails,ProductWithDetails>{
	...
	public ProductWithDetails process(ProductWithDetails p) throws Exception {
        ...
        /*
        System.out.println("Processing product id="+p.getId()+" label="+p.getLabel()+
        		" category="+p.getMain_category()+" current price="+p.getPrice());
        */

        /*
        //Bon comportement ici en mode JTA/XA:
        //l'ensemble des lignes du chunck (ici de taille 5) sera rollbacké et pas seulement cette ligne 7
		if(p.getLabel().equals("divers_7"))
            p.setId(100000);//not existing product id to simulate a writing error later
        */
