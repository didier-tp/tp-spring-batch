...
public class ProductsWithDetailsCsvToDbJobConfig extends MyAbstractJobConfig{
  ...
  public static final Logger logger = LoggerFactory.getLogger(ProductsWithDetailsCsvToDbJobConfig.class);


  @Bean
  public Job fromDetailsCsvToDbJob(@Qualifier("csvToDb") Step step1) {
    var name = "fromDetailsCsvToDbJob";//same as method name for simple mind developper
  ...
  public Step stepDetailsCsvToDb(@Qualifier("csv") ItemReader<ProductWithDetails> productItemReader,
    ...
    var name = "stepDetailsCsvToDb";//same as method name for simple mind developper
